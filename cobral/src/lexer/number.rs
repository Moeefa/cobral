use crate::{logger, shared::AppHandleManager};
use tauri::Emitter;
...
impl<'a> Lexer<'a> {
  pub fn read_number(&mut self) -> LabeledToken {
    ...
    if num_str.is_empty() {
      logger::error(LexerError::UnexpectedEOF(self.location.line));
    ...
    if num_str.matches(".").count() > 0 {
      let float_value = num_str.parse().unwrap_or_else(|_| {
        logger::error(LexerError::FloatParseError(
          self.location.line,
          num_str.clone(),
        ));
      ...
      let int_value = num_str.parse().unwrap_or_else(|_| {
        logger::error(LexerError::IntegerParseError(
          self.location.line,
          num_str.clone(),
        ));
